...
source(file.path(CodeFolder,"/lib/R/LibraryFastCall.R"))
dyn.load(file.path(CodeFolder,"/lib/F77/FastJointSLMLibraryI.so"))
source(file.path(CodeFolder,"/lib/R/LibraryJSLMIn.R"))

### Loading centromere file ### 
CentromereTable <- read.table(file.path(CodeFolder,paste("/data/centromere/CentromerePosition_",Assembly,".txt",sep="")),sep="\t",quote="",header=T)
...
for (zz in 1:length(ExpTest))
{
  ...
  ParameterVec <- as.numeric(unlist(read.table(file.path(CodeFolder,"ParameterFile.txt"),sep="\t",quote="",header=F,comment.char="#")))
