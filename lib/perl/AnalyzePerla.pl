...
#die ("User ERROR: Please set option --assembly with a valid string ('hg38' or 'hg19').\n") if( $Assembly ne "hg38" && $Assembly ne "hg19" );
...
$L1=length($myscript);
$L2=length($0);
$workingfolder=cwd($0);#substr $myscript, 0, ($L1 - $L2 - 25);

$Program_Folder_Path="$workingfolder";
$Code_folder= substr $directory, 0, -10;#

#print "Program folder is: $Program_Folder_Path\n";

die ("User ERROR: the target specified with --target wasn't found for assembly $Assembly. \nPlease check --target and --assembly. \nError found") if (! -d "$Code_folder/data/targets/$Assembly/$Target_Name" );

$R_Target_Folder="$Code_folder/data/targets/$Assembly/$Target_Name";
...
print "Starting Segmentation and Calling...\n";
 system qq(R --slave --args $Main_Output_Folder_Path,$R_Target_Folder,$Input_File_Path,$Mode,$Target_Name,$Program_Folder_Path,$Assembly,$Code_folder < $Code_folder/lib/R/EXCAVATORInferenceExome.R);
print "Segmentation and Calling Complete\n";



  system qq(R --slave --args $Main_Output_Folder_Path,$Input_File_Path < $Code_folder/lib/R/EXCAVATORPlotsExome.R);
# 
