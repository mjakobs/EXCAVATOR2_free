...
$L1=length($myscript);
$L2=length($0);
$workingfolder=cwd($0); #substr $myscript, 0, ($L1 - $L2 - 1);

$Program_Folder_Path= "$workingfolder";
$Code_folder= substr $directory, 0, -1;
...
if($Mode eq "pooling"){ 
  ...
  system qq(R --slave --args $Program_Folder_Path,$Main_Output_Folder_Path,$Code_folder/data/targets/$Assembly/$Target_Name,$Input_File_Path,$Mode,$Target_Name,$Code_folder < $Code_folder/lib/R/PoolingCreateControl.R);
...
system qq(R --slave --args $Program_Folder_Path,$Input_File_Path,$Main_Output_Folder_Path,$Target_Name,$Assembly,$processors,$Mode,$Code_folder < $Code_folder/lib/R/DataAnalysisParallel.R);
